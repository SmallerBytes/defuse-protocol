...
export function build({ view, send }) {
  ...
  group.add(pickle);
  const relTex = new CanvasTex(320, 64);
  relTex.draw((ctx, w, h) => drawLabel(ctx, w, h, 'RELEASE', { bg: '#2a2d35', color: '#cfd6e4', font: `bold 26px 'Consolas', monospace` }));
  const relLabel = new THREE.Mesh(new THREE.PlaneGeometry(0.078, 0.014), labelMaterial(relTex));
